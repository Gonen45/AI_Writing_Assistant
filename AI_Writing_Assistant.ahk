...
ShowLoading(action := "") {
    ...
    loadingGui.SetFont("s18", "Segoe UI")
    loadingGui.Add("Text", "x12 y10 w36 h36", icon)
    loadingGui.SetFont("s11 bold cE0E0FF", "Segoe UI")
    loadingGui.Add("Text", "x54 y16 w220", msg)
    loadingGui.Show("NoActivate xCenter y20 w290 h56")
}
...
ShowResultEditor(original, aiResult, title := "Review AI Result") {
    ...
    editorGui.BackColor := "F5F5F5"

    editorGui.SetFont("s9 cGray", "Segoe UI")
    editorGui.Add("Text", "x10 y8 w580", "Original:")
    origBox := editorGui.Add("Edit", "x10 y24 w580 h90 ReadOnly Wrap", original)
    origBox.SetFont("s9", "Consolas")

    editorGui.SetFont("s9 cGray", "Segoe UI")
    editorGui.Add("Text", "x10 y125 w580", "AI Result  (editable -- tweak before pasting):")
    resultBox := editorGui.Add("Edit", "x10 y142 w580 h160 Wrap", aiResult)
    resultBox.SetFont("s10", "Segoe UI")

    btnApply   := editorGui.Add("Button", "x10  y316 w100 h30 Default", "Apply")
    btnDiscard := editorGui.Add("Button", "x120 y316 w100 h30",         "Discard")
...
ExtractResult(rawResponse) {
    ; Step 1: decode \uXXXX unicode escapes (Ollama encodes Hebrew as \u05e9 etc.)
    raw := rawResponse
    while RegExMatch(raw, "\\\\u([0-9a-fA-F]{4})", &um)
        raw := StrReplace(raw, um[0], Chr(Integer("0x" . um[1])))

    ; Step 2: remove ALL newline chars (real and JSON-encoded)
    ; Ollama sometimes splits tags across lines e.g. <res\nult> -> <result>
    raw := StrReplace(raw, "\n", "")   ; JSON-encoded \n (two chars)
    raw := StrReplace(raw, "\r", "")   ; JSON-encoded \r
    raw := StrReplace(raw, "`n", "")   ; real newline char
    raw := StrReplace(raw, "`r", "")   ; real carriage return

    ; Step 3: decode escaped forward slash \/ -> /
    raw := StrReplace(raw, "\/", "/")

    ; Step 4: single regex handles both <r>...</r> and <r>...</r>
...
CallOllama(instruction, textToProcess, action := "") {
    ...
    ; nothing matched — show raw for debugging
...
RunAction(action, original) {
    ...
    switch action {
        ...
        case "summarise":
            instruction := 'You are an expert summariser.\nRead the provided text and summarise its main points clearly and concisely using bullet points. Summarise in the SAME language as the original text.\n1. Thinking phase: Extract the key information.\n2. Output phase: Return ONLY the bulleted summary inside <result> and </result> tags.'
            label := "Summary"
            paste := false

        case "explain":
            instruction := 'You are a knowledgeable expert. Explain the provided text in plain, simple language that anyone can understand. Break down jargon, acronyms, or complex concepts. Respond in the SAME language as the original text.\n1. Thinking phase: Identify what needs explanation.\n2. Output phase: Return ONLY the explanation inside <result> and </result> tags.'
            label := "Explanation"
            paste := false
